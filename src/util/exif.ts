 ...
 */
export async function getExifModifyDate(file: File) {
  const tags = getExifTags(await file.arrayBuffer());
  // console.log(tags);
...
function getExifTags(buffer: ArrayBuffer) {
  const bytes = new Uint8Array(buffer);

  if (bytes[0] !== 0xFF && bytes[1] !== 0xD8) { // Starts with JPEG magic number
    throw new Error("not a jpeg");
  }

  for (let i = 2; i < bytes.length; i++) {
    // JPEG segment start is 0xFF, 0xFF in image data is represented as 0xFF 0x00
    if (bytes[i] === 0xFF && bytes[i+1] !== 0x00) {
      const segmentType = bytes[i+1];
      if (segmentType === 0xE1) { // APP1 segment, used by Exif
        const segmentLength = new DataView(buffer).getUint16(i+2);
        const segmentBuffer = buffer.slice(i+4, i+4+segmentLength);
        const segmentBytes = new Uint8Array(segmentBuffer);
        if (String.fromCharCode(...segmentBytes.slice(0, 6)) !== "Exif\u0000\u0000") {
          // console.log(`APP1 segment at ${i} isn't Exif, ignoring segment`);
          continue;
        }
        
        const tiffBuffer = segmentBuffer.slice(6);
